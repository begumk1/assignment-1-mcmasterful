...
const FilterSchema = z
    ...
    .refine(
        ...
        {
            message:
                "Invalid filter: at least one condition must be defined, and 'from' must be <= 'to' when both are provided",
        }
    );
    ...
    async (ctx: Context) => {
        try {
            ...
            if (validatedFilters && validatedFilters.length > 0) {
                books = await dbService.getBooksByFilters(
                    validatedFilters as Filter[]
                );
            } else {
...
router.put(
    ...
    async (ctx: Context) => {
        try {
            ...
            if (validatedBook.name !== undefined)
                updateData.name = validatedBook.name;
            if (validatedBook.author !== undefined)
                updateData.author = validatedBook.author;
            if (validatedBook.description !== undefined)
                updateData.description = validatedBook.description;
            if (validatedBook.price !== undefined)
                updateData.price = validatedBook.price;
            if (validatedBook.image !== undefined)
                updateData.image = validatedBook.image;
